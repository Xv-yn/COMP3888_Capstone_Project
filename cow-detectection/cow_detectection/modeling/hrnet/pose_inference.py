"""
MMPose-compatible helpers for initializing a pose model, running inference
(top-down and bottom-up), visualizing keypoints, and converting detector
outputs to person bbox lists.

Contents:
    - init_pose_model:  build & load a posenet from a config + checkpoint
    - inference_top_down_pose_model:  run KPT on per-person bboxes
    - inference_bottom_up_pose_model: run KPT on the whole image (no bboxes)
    - vis_pose_result:  draw poses (keypoints + links) on an image
    - process_mmdet_results: convert mmdet outputs to a list of 'person' dicts
"""
...
class LoadImage:
    ...
    def __call__(self, results):
        ...
        if isinstance(results["img_or_path"], str):
            ...
            img = mmcv.imread(results["img_or_path"], self.color_type, self.channel_order)
